...
use simpfun_sdk::{SimpfunClient, SdkError, WsEvent, connect_ins_ws};
        ...
        match connect_ins_ws(&client, instance_id).await {
            Ok((rx, control, _stop)) => {
                ...
                while let Ok(event) = rx.recv_async().await {
                    match event {
                        ...
                        WsEvent::TokenExpiring => println!("[Token] 即将过期，正在自动刷新..."),
            ...
            Err(e) => {
                ...
                if let SdkError::AuthFailed = e {
                    ...
                    loop {
                        match client.login_and_set_token(&username, &password).await {
                            Ok(_) => {
                                println!("重新登录成功！");
                                break;
                            }
                            Err(login_err) => {
                                eprintln!("重新登录失败: {}, {}秒后重试...", login_err, retry_interval);
