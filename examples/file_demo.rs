...
async fn main() -> Result<(), SdkError> {
    ...
    // 1. 列出根目录文件
    println!("--- ins_file_list (根目录) ---");
    let root_files = client.ins_file_list(instance_id, "/").await?;
    println!("文件/目录数量: {}", root_files.list.len());
    for file in root_files.list.iter().take(10) {
        let type_str = if file.file { "文件" } else { "目录" };
        let size_str = file.size.map(|s| format!("{} B", s)).unwrap_or_else(|| "-".to_string());
        println!(
            "  [{}] {} | 大小: {} | 修改: {:?}",
    ...
    // 2. 列出特定目录
    println!("\n--- ins_file_list (/home/container) ---");
    match client.ins_file_list(instance_id, "/home/container").await {
        Ok(files) => {
            println!("文件/目录数量: {}", files.list.len());
            for file in files.list.iter().take(10) {
                println!("  {} ({})", file.name, if file.file { "文件" } else { "目录" });
            }
        }
    ...
    for path in config_files {
        match client.ins_file_fetch(instance_id, path).await {
            Ok(content) => {
                let preview = if content.content.len() > 200 {
                    format!("{}... (共 {} 字符)", &content.content[..200], content.content.len())
                } else {
    ...
    // 保存文件
    // println!("\n--- ins_file_save ---");
    // let save = client.ins_file_save(instance_id, "/home/container/test.txt", "Hello World!").await?;
    // println!("保存结果: {:?}", save.msg);

    // 创建文件
    // println!("\n--- ins_file_create (文件) ---");
    // let create_file = client.ins_file_create(instance_id, "file", "/home/container", "new_file.txt").await?;
    // println!("创建结果: {:?}", create_file.msg);

    // 创建目录
    // println!("\n--- ins_file_create (目录) ---");
    // let create_dir = client.ins_file_create(instance_id, "folder", "/home/container", "new_folder").await?;
    // println!("创建结果: {:?}", create_dir.msg);

    // 重命名文件
    // println!("\n--- ins_file_rename ---");
    // let rename = client.ins_file_rename(instance_id, "/home/container/old.txt", "/home/container/new.txt").await?;
    // println!("重命名结果: {:?}", rename.msg);

    // 删除文件
    // println!("\n--- ins_file_delete ---");
    // let delete = client.ins_file_delete(instance_id, vec![
    ...
    // 压缩文件
    // println!("\n--- ins_file_archive ---");
    // let archive = client.ins_file_archive(
    //     instance_id,
    //     "/home/container",
    ...
    // 解压文件
    // println!("\n--- ins_file_unarchive ---");
    // let unarchive = client.ins_file_unarchive(instance_id, "/home/container", "archive.zip").await?;
    // println!("解压结果: {:?}", unarchive.msg);

    // 粘贴文件
    // println!("\n--- ins_file_paste ---");
    // let paste = client.ins_file_paste(
    //     instance_id,
    ...
    // 复制文件（复制到剪贴板）
    // println!("\n--- ins_file_copy ---");
    // let copy = client.ins_file_copy(instance_id, "/home/container/file.txt").await?;
