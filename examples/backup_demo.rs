...
//!
//! 演示以下 API：
//! - user().ins_backup_list: 备份列表
//! - user().ins_backup_create: 创建备份
//! - user().ins_backup_rename: 重命名备份
//! - user().ins_backup_restore: 恢复备份
//! - user().ins_backup_delete: 删除备份
//! - user().ins_backup_download: 获取下载链接
//! - user().ins_rollback_points: 回滚时间点列表
//! - user().ins_rollback_create: 执行回滚
...
async fn main() -> Result<(), SdkError> {
    ...
    // 2. 备份列表
    println!("--- user().ins_backup_list ---");
    let backups = client.user().ins_backup_list(instance_id).await?;
    ...
    // 3. 回滚时间点列表
    println!("\n--- user().ins_rollback_points ---");
    match client.user().ins_rollback_points(instance_id).await {
        Ok(points) => {
    ...
    // 创建备份
    // println!("\n--- user().ins_backup_create ---");
    // let create = client
    //     .user()
    //     .ins_backup_create(instance_id, "手动备份")
    //     .await?;
    // println!("创建结果: {:?}", create.msg);

    // 重命名备份
    // if let Some(backup_id) = first_backup_id {
    //     println!("\n--- user().ins_backup_rename ---");
    //     let rename = client
    //         .user()
    //         .ins_backup_rename(instance_id, backup_id, "新名称")
    //         .await?;
    ...
    // 恢复备份
    // if let Some(backup_id) = first_backup_id {
    //     println!("\n--- user().ins_backup_restore ---");
    //     let restore = client
    //         .user()
    //         .ins_backup_restore(instance_id, backup_id)
    //         .await?;
    ...
    // 删除备份
    // if let Some(backup_id) = first_backup_id {
    //     println!("\n--- user().ins_backup_delete ---");
    //     let delete = client
    //         .user()
    //         .ins_backup_delete(instance_id, backup_id)
    //         .await?;
    //     println!("删除结果: {:?}", delete.msg);
    // }

    // 执行回滚
    // println!("\n--- user().ins_rollback_create ---");
    // let rollback = client
    //     .user()
    //     .ins_rollback_create(instance_id, "2024-01-15T10:00:00")
    //     .await?;
