...
pub async fn connect_ins_ws(client: &SimpfunClient, id: i64) -> Result<(flume::Receiver<WsEvent>, WsControl, WsStop), SdkError> {
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            // 1. 初始化 (获取 Token & URL)
            let (token, socket_url) = match client_clone.ins_ws_init(id).await {
                Ok(init) => (init.data.token, init.data.socket),
                Err(e) => {
                    let _ = tx_evt.send_async(WsEvent::Error(format!("获取WS信息失败: {}", e))).await;
                    let _ = tx_evt.send_async(WsEvent::Reconnecting(format!("等待 {}s 后重试...", backoff_secs))).await;
            ...
            match connect_async(&socket_url).await {
                Ok((ws_stream, _)) => {
                            ...
                            ws_msg = ws_read.next() => {
                                match ws_msg {
                                    Some(Ok(msg)) => {
                                        match msg {
                                            Message::Text(t) => {
                                                process_ws_message(&t, &tx_evt, &client_clone, &tx_msg, id, &socket_url, &refreshing).await;
                                            }
                                            Message::Binary(_) => {}
                                            Message::Ping(d) => {
                    ...
                    let _ = tx_evt.send_async(WsEvent::Reconnecting(format!("连接断开，{}s 后重试...", backoff_secs))).await;
                    sleep(Duration::from_secs(backoff_secs)).await;
                    backoff_secs = (backoff_secs * 2).min(max_backoff);
                }
                Err(e) => {
...
async fn process_ws_message(
    ...
    if let Ok(v) = serde_json::from_str::<WsPacket>(text) {
        ...
        match ev {
            ...
            "token expiring" => {
                ...
                tokio::spawn(async move {
                    ...
                    match client_inner.ins_ws_init(id).await {
                         Ok(new_init) => {
                         ...
                         Err(e) => {
                             let _ = tx_evt_inner.send_async(WsEvent::TokenRefreshFailed(e.to_string())).await;
                         }
                    }
        ...
        warn!("解析WS文本失败: {}", text);
        let _ = tx_evt.send_async(WsEvent::Error("解析WS文本失败".to_string())).await;
    }
}
