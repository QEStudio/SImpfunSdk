...
impl<'a> UserClient<'a> {
    ...
    pub async fn shop_list(&self, version_id: i64) -> Result<ShopListResponse, SdkError> {
        ...
        let resp = self.inner.send_with_retry(|| self.inner.http.get(url.clone()).headers(headers.clone())).await?;
        let sr: ShopListResponse = resp.json().await?;
        if sr.code != 200 {
            return Err(SdkError::Api { code: sr.code, msg: "获取镜像类别失败".to_string() });
        }
        Ok(sr)
    }
}
