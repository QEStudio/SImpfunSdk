...
impl Default for PollingConfig {
    fn default() -> Self {
        Self { fast_secs: 1, safety_secs: 60 }
    }
}
...
impl EventControl {
    ...
    pub async fn force_refresh_topics(&self, topics: Vec<Topic>) {
        let _ = self.tx.send_async(EventCmd::ForceRefresh { topics: Some(topics) }).await;
    }
...
impl EventManager {
    pub fn new(client: SimpfunClient) -> Self {
        Self { client, watch_details: Vec::new() }
    }
    ...
    pub async fn start(self, intervals: Option<EventIntervals>) -> Result<(flume::Receiver<Event>, EventStop), SdkError> {
...
async fn fetch_and_dispatch(
    ...
    match topic {
        Topic::Announcement => {
            let v = client.announcement_list().await?;
        ...
        Topic::Point => {
            let v = client.point_history().await?;
        ...
        Topic::Diamond => {
            let v = client.diamond_history().await?;
        ...
        Topic::Invite => {
            let v = client.invite_info().await?;
...
async fn fetch_and_dispatch_detail(
    ...
    let v = client.ins_detail(id).await?;
